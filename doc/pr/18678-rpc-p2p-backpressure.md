...
**Issue:** https://github.com/bitcoin/bitcoin/issues/18678

## Motivation

Under sustained low-priority P2P traffic (tx relay, addr gossip), RPC tail latency can degrade significantly, especially on resource-constrained nodes. The current message handler processes all P2P messages without considering RPC queue pressure, leading to RPC starvation scenarios.

From issue #18678:
> "RPC calls can experience significant latency spikes when the node is processing heavy P2P traffic"

This affects users running both RPC-heavy workloads (wallets, block explorers, Lightning nodes) and participating in P2P network relay.
...
### Unit Tests (`src/test/stdio_bus_hooks_tests.cpp`)
...
- Issue: https://github.com/bitcoin/bitcoin/issues/18678
- Related: #21803 (block processing delays), #27623 (message handler saturation)
